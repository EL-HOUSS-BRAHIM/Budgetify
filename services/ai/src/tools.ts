...
export const ToolDefinitions = [
  {
    ...
    parameters: z.object({
      ...
      category: z.string().describe('The category name (e.g. Food & Dining, Groceries, Transport, Entertainment, Utilities)'),
      description: z.string().describe('Description or merchant name of the expense'),
      date: z.string().optional().describe('ISO date string of when the expense occurred (defaults to now)'),
    }),
  },
  {
    name: 'record_income',
  ...
  },
  {
    ...
    parameters: z.object({
      title: z.string().describe('Title of the planned item or bill (e.g. Internet Bill, Dental checkup, Car maintenance)'),
      ...
      is_unplanned: z.boolean().default(false).describe('Set to true if this was an unexpected/unplanned expense added on the fly'),
    }),
  },
  {
    ...
    parameters: z.object({
      title_or_id: z.string().describe('The title or ID of the planned item to mark (e.g. "Internet", "Rent")'),
